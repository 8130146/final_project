...
@st.cache_resource
def load_dataset():
    data = pd.read_csv('../datasets/dataset_series.csv')
    return data
...
def generate_dtree_graph(best_model, X):
    ...
    # Guardar o grafico
    folder_path = "../images"
    graph.render(os.path.join(folder_path, "grafico_decision_tree_series"),
                 format='png')
...
def decisiontree():
    ...
    calc_save_metrics(Y_test, y_pred, 'metrics_decision_tree_series.pkl')
    save_model(best_model, 'decision_tree_model_series.pkl')
    generate_dtree_graph(best_model, X)
    generate_shap_plot_dTree(best_model, X_test, 'shap_dTree_series')
    generate_lime_plot(best_model, X_train, X_test, X, 'lime_dTree_series')
...
def plot_feature_distributions(X_train, Y_train):
    ...
    for feature_name in X_train.columns:
        ...
        fig.tight_layout()
        fig.savefig(f'../images/distribuicao_{feature_name}_series.png')
        plt.close(fig)
...
def naivebayes():
    ...
    param_dist = {'var_smoothing': np.logspace(-9, 0, num=20)}
    random_search = RandomizedSearchCV(
    ...
    # Guardar tudo
    save_model(nb, 'naive_bayes_model_series.pkl')
    calc_save_metrics(Y_test, y_pred_adjusted,
                      'metrics_naive_bayes_series.pkl')
    file_path_data = os.path.join('../metrics', 'data_naive_bayes_series.pkl')
    ...
    # generate_shap_plot_nb(nb, X_train, X_test, 'shap_naiveBayes_series')
    generate_lime_plot(nb, X_train, X_test, X, 'lime_naiveBayes_series')
